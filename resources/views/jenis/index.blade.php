          ...
          <tr>
            ...
            <td>
              <div class="d-flex gap-1">
                ...
                <form method="POST" action="{{ route('jenis.destroy', $item->id) }}" class="d-inline" id="deleteForm-{{ $item->id }}">
                  @csrf
                  @method('DELETE')
                  <button type="button" class="btn btn-danger" onclick="confirmDelete('deleteForm-{{ $item->id }}', 'Jenis \'{{ $item->nama }}\' akan dihapus secara permanen.')">Hapus</button>
                </form>
              </div>
            </td>
          </tr>
